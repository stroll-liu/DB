...
export const runSteps = (steps, ...args) => {
  for (let fn of steps) { fn(...args); }
};

export const runInEnd = (in_end, groups) => {
  for (let group_doc of groups) {
    runSteps(in_end, group_doc);
  }
};

export const groupLoopFn = (next, in_end, groups, fn) => (cb) => {
  const done = (error) => {
...
export const createGroupByRefFn = (next, expr, steps) => {
...
export const createGroupFn = (next, expr, steps) => {
  if (expr.has_refs) {
